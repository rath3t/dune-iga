...
void drawElements(const GridView& gridView, double lineWidth = 1, std::string&& lineColor = "black",
    ...
    for (auto& element : elements(gridView)) {
        ...
        for (size_t edgeIndex = 0; edgeIndex < element.subEntities(edgeCodim); ++edgeIndex) {
            ...
            auto l = ax->plot(edgeCoords[0], edgeCoords[1]);
...
void plotGridViews(const std::shared_ptr<Dune::IGA::NURBSGrid<2, 2>>& grid, std::string&& postfix = "") {
    ...
    Plot::plotGridView(paraGridView, "plot" + postfix + "/parametricGrid");
...
void plotEveryReconstructedGrid(const std::shared_ptr<Dune::IGA::NURBSGrid<2, dimworld>>& grid, std::string&& postfix = "") {
  for (int i = 0; auto& ele : elements(grid->leafGridView())) {
      if (ele.impl().getTrimFlag() == Dune::IGA::ElementTrimFlag::trimmed) {
        ...
        Plot::plotGridView(gV, "plot" + postfix + "/grid_" + std::to_string(i));
      }
      ++i;
  }
}

void saveEveryReconstructedGrid(const std::shared_ptr<Dune::IGA::NURBSGrid<2, 2>>& grid) {
  for (int i = 0; auto& ele : elements(grid->leafGridView())) {
      if (ele.impl().getTrimFlag() == Dune::IGA::ElementTrimFlag::trimmed) {
        auto gV = grid->getPatch().getTrimmedElementRepresentation(i)->gridView();
        Dune::VTKWriter vtkWriter(gV);
        vtkWriter.write("grid_" + std::to_string(i));
      }
      ++i;
  }
}
