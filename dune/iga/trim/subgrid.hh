  ...
  class TrimmedSubGrid {
   ...
   private:
    ...
    void checkAndDivideSmallLoops() {
      ...
      auto partsForSize = [](std::size_t size) -> int {
        return size == 1 ? 4 : 2;
      };
   ...
   private:
    ...
    auto splitOuterBoundaries(int maxDivisions = maxOuterBoundaryDivisions, double targetTolerance = outerTargetTolerance) const  {
      return splitBoundariesImpl(outerBoundaries_, maxDivisions, targetTolerance, transformer);
    }

    auto splitInnerBoundaryLoops(int maxDivisions = maxInnerBoundaryDivisions, double targetTolerance = innerTargetTolerance) const -> decltype(innerBoundaries_) {
      if (innerBoundaries_) {
   ...
   public:
    auto createRefinedGrid(int refinementSteps) const -> std::tuple<decltype(elements_), decltype(vertices_), decltype(indices_)> {
      ...
      // Attention: This may break down for other grid implementations, as the insertion Index doesn't have to be the "real" entity index
      ...
      auto determineInnerBoundaryIndices = [](unsigned int bIndex, unsigned int nBoundariesInLoop, unsigned int innerIndex) {
