...
public:
  ...
  [[nodiscard]] Volume volume() const {
    ...
    const auto& rule = Dune::QuadratureRules<ctype, mydimension>::rule(GeometryTypes::cube(mydimension),
                                                                       (*std::ranges::max_element(patchData_->degree)));
    ctype vol        = 0.0;
    for (auto& gp : rule)
      vol += integrationElement(gp.position()) * gp.weight();
    return vol;
  ...
  auto getDirectionsOfSubEntityInParameterSpace() const {
    ...
    for (int subI = 0, i = 0; i < griddim; ++i) {
      if constexpr (mydimension != griddim)
        if (fixedOrVaryingDirections_[i] == Dune::IGA::Impl::FixedOrFree::fixed)
          continue;
      subDirs[subI++] = i;
    }
    return subDirs;
