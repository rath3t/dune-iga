...
namespace Dune::IGANEW::Utilities {
   ...
   * \tparam dim2 only a dummy dim due to the size_t and int size mismatch for FieldVector and std::array
  ...
  requires(dim == dim2) bool clampToBoundaryAndCheckIfIsAtAllBoundaries(
      FieldVector<ScalarType, dim>& x, const std::array<Domain<ScalarType>, dim2>& domain = {}) {
