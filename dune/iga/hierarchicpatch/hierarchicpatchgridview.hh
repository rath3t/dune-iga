...
namespace Dune::IGANEW
{
  ...
  struct PatchGridLevelGridView : public DefaultLevelGridView<const GridImp>
  {
    ...
    PatchGridLevelGridView(const typename DefaultLevelGridView<const GridImp>::Grid& grid, int level ): DefaultLevelGridView<const GridImp>(grid,level){}
...
return this->grid().patchGeometries[this->level_].patchData();
  ...
  struct PatchGridLeafGridView : public DefaultLeafGridView<const GridImp>
  {
    ...
    PatchGridLeafGridView(const typename DefaultLeafGridView<const GridImp>::Grid& grid): DefaultLeafGridView<const GridImp>(grid){}

    static constexpr Trimming trim = GridImp::trim;
    const auto& patchData() const
    {
      return this->grid().patchGeometries[this->grid().maxLevel()].patchData();
    }
