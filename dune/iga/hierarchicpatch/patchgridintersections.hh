  ...
  class PatchGridLeafIntersection {
   ...
   public:
    ...
    PatchGridLeafIntersection(const GridImp* parameterSpaceGrid, const ParameterSpaceLeafIntersection& hostIntersection)
        : patchGrid_(parameterSpaceGrid), parameterSpaceIntersection(hostIntersection) {}
    ...
    Geometry geometry() const {
      ...
      auto geo = typename Geometry::Implementation(
          parameterSpaceIntersection.geometry(), patchGrid_->patchGeometries_.back().template localView<1, Trimmer>());
      return Geometry(geo);
    }
