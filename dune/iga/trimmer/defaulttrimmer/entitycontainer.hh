...
namespace Dune::IGANEW::DefaultTrim {
  ...
  struct VectorEntityContainer {
    ...
    template <int codim>
    requires(codim >= 0 and codim <= 2) auto& entity(const IdType& id, int lvl) {
    ...
    GeoTypes types(int codim, int level) const {
      if (codim == 0)
        return numberOfTrimmedElements[level] == 0
                   ? GeoTypes{GeometryTypes::cube(gridDim)}
    ...
    std::size_t size(GeometryType type, int lvl) const {
      if (type == GeometryTypes::quadrilateral) return numberOfUnTrimmedElements[lvl];
      if (type == GeometryTypes::none(gridDim)) return numberOfTrimmedElements[lvl];
      if (type == GeometryTypes::line) return std::get<1>(entityImps_[lvl]).size();
      if (type == GeometryTypes::vertex) return std::get<2>(entityImps_[lvl]).size();
      return 0;
    }
