...
namespace Dune::IGANEW::Trim {
  ...
  class ParameterSpaceGridEntityVariant {
   public:
    ...
    template <class Implementation>
    ParameterSpaceGridEntityVariant(const Implementation& impl) : impl_(impl) {}
    ...
    template <class Implementation>
    requires(!std::is_same_v<Implementation, ParameterSpaceGridEntityVariant>) ParameterSpaceGridEntityVariant&
    operator=(const Implementation& impl) {
      impl_ = impl;
      return *this;
    };
     ...
     */
    template <int cc>
    requires(codim_ == 0) [[nodiscard]] decltype(auto) subEntity(int i) const {
    ...
    template <typename = void>
    requires(codim_ == 0) decltype(auto) ilevelend() const {
      return visit([](const auto& impl) { return impl.ilevelend(); });
    }

    //! First leaf intersection
    template <typename = void>
    requires(codim_ == 0) decltype(auto) ileafbegin() const {
      return visit([](const auto& impl) { return impl.ileafbegin(); });
    }

    //! Reference to one past the last leaf intersection
    template <typename = void>
    requires(codim_ == 0) decltype(auto) ileafend() const {
      return visit([](const auto& impl) { return impl.ileafend(); });
    }

    //! returns true if Entity has NO children
    template <typename = void>
    requires(codim_ == 0) bool isLeaf() const {
     ...
     */
    template <typename = void>
    requires(codim_ == 0) decltype(auto) geometryInFather() const {
     ...
     */
    template <typename = void>
    requires(codim_ == 0) decltype(auto) hbegin(int maxLevel) const {
      return visit([&](const auto& impl) { return impl.hbegin(maxLevel); });
    }

    //! Returns iterator to one past the last son
    template <typename = void>
    requires(codim_ == 0) decltype(auto) hend(int maxLevel) const {
      return visit([&](const auto& impl) { return impl.hend(maxLevel); });
    }

    //! @todo Please doc me !
    template <typename = void>
    requires(codim_ == 0) bool wasRefined() const {
