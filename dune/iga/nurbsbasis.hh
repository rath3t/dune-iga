 ...
 * \tparam R Number type used for spline function values
   ...
   * The patch object does all the work.
       */
      NurbsLocalBasis(const NurbsPreBasis<GV,MI>& preBasis,
                        const NurbsLocalFiniteElement<GV,R,MI>& lFE)
          : preBasis_(preBasis),
            lFE_(lFE)
      {}
 ...
 * \tparam MI Global multi-index type.  Only here for technical reasons
    ...
    public:
       ...
       */
      typedef LocalFiniteElementTraits<NurbsLocalBasis<GV,R,MI>,
                                       NurbsLocalCoefficients<dim>,
                                       NurbsLocalInterpolation<dim,NurbsLocalBasis<GV,R,MI> > > Traits;

      /** \brief Constructor with a given B-spline basis
       */
      NurbsLocalFiniteElement(const NurbsPreBasis<GV,MI>& preBasis)
          : preBasis_(preBasis),
 ...
 * in a larger basis for the construction of product spaces.
   ...
   *        the end of the parameter interval.
       */
      NurbsPreBasis(const GridView& gridView,
                      const std::array<std::vector<double>,dim>& knotVector,
                    const std::array<int,dim>&  order,
                      bool makeOpen = true)
          : gridView_{gridView},
            knotVectors_{knotVector},
            order_{order}
      {
        auto uniqueKnotVector = knotVector;
        for(int i=0; i<dim ; ++i )
          uniqueKnotVector[i].erase(std::ranges::begin(std::ranges::unique(uniqueKnotVector[i])), std::end(uniqueKnotVector[i]));
        std::ranges::transform(uniqueKnotVector,elements_.begin(),std::identity{},&std::vector<double>::size);
      }
   ...
   * \deprecated
      ...
      [[nodiscard]] unsigned int size (size_t d) const
      {
...
//        auto N = Nurbs<dim>::basisFunctions(in,)
   ...
   * \param [out] outHess Vector containing the second derivatives of all B-spline derivatives at 'in' (only if calculation was switched on by enableEvaluations)
    ...
    public:
      ...
      NurbsNode(const NurbsPreBasis<GV, MI>* preBasis) :
                                                             preBasis_(preBasis),
    ...
    namespace BasisFactory {
      ...
      namespace Imp {
        ...
        public:
          ...
          NurbsPreBasisFactory(const std::array<std::vector<double>,dim>& knotVector,
                               const std::array<int,dim>& order,
                                 bool makeOpen = true)
          ...
          {
            return NurbsPreBasis<GridView, MultiIndex>(gridView, knotVector_, order_, makeOpen_);
          }

        private:
          const std::array<std::vector<double>,dim>& knotVector_;
 ...
 *
       */
       template<int dim>
      auto nurbs(const std::array<std::vector<double>,dim>& knotVector,
                 const std::array<int,dim>& order,
                   bool makeOpen = true)
      {
        return Imp::NurbsPreBasisFactory<dim>(knotVector, order, makeOpen);
      }
