...
namespace Dune::IGA {
  ...
  template <std::integral auto dim, std::integral auto dimworld,
            LinearAlgebra NurbsGridLinearAlgebraTraits = DuneLinearAlgebraTraits<double>> requires (dim == 2)
  class NURBSTrimmedPatch {
   public:
    ...
    NURBSTrimmedPatch(
        ...
        const typename NURBSTrimmedPatchData<dim, dimworld, NurbsGridLinearAlgebraTraits>::ControlPointNetType controlPoints,
        const std::array<int, dim> degree)
        : NURBSTrimmedPatch(NURBSTrimmedPatchData<dim, dimworld, NurbsGridLinearAlgebraTraits>(knotSpans, controlPoints, degree)) {}

    explicit NURBSTrimmedPatch(const NURBSTrimmedPatchData<dim, dimworld, NurbsGridLinearAlgebraTraits>& patchData)
        : patchData_{std::make_shared<NURBSTrimmedPatchData<dim, dimworld, NurbsGridLinearAlgebraTraits>>(patchData)} {
   ...
   private:
    ...
    std::shared_ptr<NURBSTrimmedPatchData<dim, dimworld, NurbsGridLinearAlgebraTraits>> patchData_;
