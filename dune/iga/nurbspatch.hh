...
public:
  ...
  [[nodiscard]] int numBoundarySegments() const {
    if constexpr (dim == 1)
      ...
      if (trimData_.has_value())
        return boundarySegmentList().size();
      return (validKnotSize()[0] + validKnotSize()[1]) * 2;
    } else if constexpr (dim == 3)
      return 2 * (validKnotSize()[0] * validKnotSize()[1] + validKnotSize()[1] * validKnotSize()[2] +
  ...
  std::pair<std::array<int, dim>, std::array<Impl::FixedOrFree, dim>> spanAndDirectionFromDirectIndex(
    ...
    } else if constexpr (dim - codim == 1 && dim > 1) // edge case
    {
      ...
      spanIndices[edgeDir] =
          std::fmod(std::floor(indexInDir / (std::accumulate(uniqueSpanSize_.begin(), uniqueSpanSize_.begin() + edgeDir,
                                                             1, [](auto res, auto r) { return res * (r + 1); }))),
    ...
    } else if constexpr (dim - codim == 2 && dim > 2) // surface case
    {
      ...
      spanIndices[surfFixedDir] = std::fmod(
          std::floor(indexInDir / (std::accumulate(uniqueSpanSize_.begin(), uniqueSpanSize_.begin() + surfFixedDir, 1,
                                                   std::multiplies{}))),
      ...
      if (surfFixedDir == 0) {
        ...
        if (surfFixedDir == 1)
          spanIndices[2] =
              std::fmod(std::floor(indexInDir / ((uniqueSpanSize_[1] + 1) * uniqueSpanSize_[0])), uniqueSpanSize_[2]);
        else if (surfFixedDir == 2)
  ...
  typename GridImpl::template Codim<codim>::Geometry geometry(const RealIndex realIndex) const {
    ...
    auto geo =
        (codim == 0 && trimData_ && isTrimmed(realIndex))
            ? NURBSGeometry<dim - codim, dimworld, const GridImpl>(patchData_, fixedOrFreeDirection, currentKnotSpan,
                                                                   subGridInfoMap.at(directIndex).subgrid)
            : NURBSGeometry<dim - codim, dimworld, const GridImpl>(patchData_, fixedOrFreeDirection, currentKnotSpan);
...
private:
  ...
  [[nodiscard]] int getGlobalEdgeIndexFromElementIndex(const int realIndex, const int eI,
    ...
    if constexpr (dim == 3) {
      if (edgeDir == 0)
        dIndex +=
            eleI[0] + (uniqueSpanSize_[0]) * (eleI[1]) + (uniqueSpanSize_[0]) * (uniqueSpanSize_[1] + 1) * (eleI[2]);
      else if (edgeDir == 1)
  ...
  [[nodiscard]] int getGlobalSurfaceIndexFromElementIndex(const int realIndex, const int eI) const {
    ...
    if (surfaceFixedDir == 0)
      dIndex +=
          eleI[0] + (uniqueSpanSize_[0] + 1) * (eleI[1]) + (uniqueSpanSize_[0] + 1) * (uniqueSpanSize_[1]) * (eleI[2]);
...
public:
  ...
  auto parameterSpaceGrid(int scale = 0) const {
    ...
    if constexpr (std::is_same<double, ctype>::value)
      return std::make_unique<ParameterSpaceGrid>(uniqueKnots);
  ...
  [[nodiscard]] bool isTrimmed(RealIndex realIndex) const { return getTrimFlag(realIndex) == ElementTrimFlag::trimmed; }
