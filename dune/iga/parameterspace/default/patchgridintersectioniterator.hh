...
namespace Impl {
  ...
  namespace IntersectionIteratorTraits {
    ...
    template <class GridImp>
    using EntityInfo = typename GridImp::ParameterSpace::ParameterSpaceTraits::template Codim<0>::EntityInfo;

    template <class GridImp>
    using LocalEntity = typename GridImp::ParameterSpace::template Codim<0>::ParameterSpaceGridEntity;
  ...
  private:
    auto findLocalIntersection(const auto& edgeInfo, const auto& edgeTrimInfo,
      ...
      for (const auto i : Dune::range(4)) {
        auto trimmedIdx = Transformations::mapToParameterSpace(1, i);
