...
template <class Basis>
void assemblePoissonProblem(const Basis& basis, BCRSMatrix<double>& matrix, BlockVector<double>& b,
...
int main(int argc, char* argv[]) {
  ...
  auto predicate = [](auto x) { return x[0] < 1e-8 || x[1] < 1e-8 || (x[0] > 0.4999 && x[1] > 0.4999); };
  ...
  Modify Dirichlet rows
    ...
    if (dirichletNodes[i]) {
      = stiffnessMatrix[i].begin();
      auto cIt auto cEndIt = stiffnessMatrix[i].end();
  ...
  auto dirichletValues = [](auto x) { return (x[0] < 1e-8 || x[1] < 1e-8) ? 0 : 0.5; };
