#ifndef DUNE_IGA_BSPLINEBASIS_HH
#define DUNE_IGA_BSPLINEBASIS_HH
...
class BSplineBasisLeafNode
{
  ...
  typedef BSplineLocalFiniteElement<GV,double> FiniteElement;
...
protected:
                         ...
                         const std::array<uint,dim>& currentKnotSpan) const
  {
    ...
    for (size_t i=0; i<dim; i++)
      evaluateFunctionFull(in[i], values[i], knotVectors_[i], order_[i], currentKnotSpan[i], offset[i]);
    ...
    for (size_t i=0; i<dim; i++)
    {
      ...
      else
      {
        ...
        {
          R derivativeAddend1 = values[i][order_[i]-1][j] / (knotVectors_[i][j+order_[i]]-knotVectors_[i][j]);
          R derivativeAddend2 = values[i][order_[i]-1][j+1] / (knotVectors_[i][j+order_[i]+1]-knotVectors_[i][j+1]);
    ...
    for (int i=0; i<dim; i++)
    {
      ...
      for (size_t j=0; j<limits[i]; j++)
        oneDValuesShort[i][j] = values[i][order_[i]][offset[i] + j];
    }
                                   ...
                                   unsigned int offset)
  {
    ...
    // for (size_t i=0; i<knotVector.size()-1; i++)
    //   N[0][i] = (knotVector[i] <= in) and (in < knotVector[i+1]);
    for (size_t i=offset; i<knotVector.size()-1; i++)
      N[0][i] = (i == currentKnotSpan);

    for (size_t r=1; r<=order; r++)
      for (size_t i=offset; i<knotVector.size()-r-1; i++)
      {
        R factor1 = ((knotVector[i+r] - knotVector[i]) > 1e-10)
        ? (in - knotVector[i]) / (knotVector[i+r] - knotVector[i])
        : 0;
...
#endif   // DUNE_IGA_BSPLINEBASIS_HH
