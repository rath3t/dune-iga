...
#ifndef DUNE_PYTHON_GRID_RANGE_MOD_HH
#define DUNE_PYTHON_GRID_RANGE_MOD_HH
...
struct PyIteratorMod
{
  PyIteratorMod ( Iterator begin, Iterator end )
...
// registerPyIteratorMod
// ------------------

template< class Iterator >
void registerPyIteratorMod( pybind11::handle scope, pybind11::class_< Iterator > cls )
{
...
using PyGridViewIterator = PyIteratorMod< typename GridView::template Codim< codim >::Iterator, typename GridView::template Codim< codim >::Entity >;
...
using PyGridViewPartitionIterator = PyIteratorMod< typename GridView::template Codim< codim >::template Partition< partition >::Iterator, typename GridView::template Codim< codim >::Entity >;
...
using PyIntersectionIterator = PyIteratorMod< typename GridView::IntersectionIterator, typename GridView::Intersection >;
...
-> std::enable_if_t< Capabilities::canIterate< typename GridView::Grid, codim >::value >
{
  ...
  if( entry.second )
  {
    registerPyIteratorMod( scope, entry.first );
...
inline static void registerPyIntersectionIterator ( pybind11::handle scope = {} )
{
  ...
  if( entry.second )
    registerPyIteratorMod< Iterator >( scope, entry.first );
}
...
inline static void registerGridViewPartition ( pybind11::handle scope = {} )
{
  ...
  if( !pybind11::already_registered< Partition >() )
  {
    ...
    std::array< pybind11::object (*) ( pybind11::object ), GridView::dimension+1 > makePyIteratorMods;
    Hybrid::forEach( std::make_integer_sequence< int, GridView::dimension+1 >(), [ &makePyIteratorMods ] ( auto codim ) {
      makePyIteratorMods[ codim ] = makePyGridViewPartitionIterator< GridView, codim, partition >;
    } );
    cls.def( "entities", [ makePyIteratorMods ] ( pybind11::object self, int codim ) {
      ...
      return makePyIteratorMods[ codim ]( self );
    }, "codim"_a );
...
#endif // #ifndef DUNE_PYTHON_GRID_RANGE_MOD_HH
